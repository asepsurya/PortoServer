...
@section('container')
    <style>
        ...
        .modal-backdrop.show {
            z-index: 1999 !important; /* backdrop tetap di bawah modal */
        }
                ...
                <div class="col-lg-8">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" id="title" value="{{ $project->title }}"
                            class="form-control form-control-lg border-0 border-bottom rounded-0 shadow-none fw-semibold"
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card border-0 shadow-sm mb-3">
                        <div class="card-body">
                            ...
                            <input type="text" id="slug" name="slug" value="{{ $project->slug }}"
                                class="form-control" placeholder="slug-project">
                                <input type="text" name="slug_old"  value="{{ $project->slug }}" hidden>
                        </div>
                    </div>
                    ...
                    <div class="card border-0 shadow-sm mb-3">
                        <div class="card-body">
                            ...
                            <select name="category_id[]" class="form-select select2" multiple size="5" id="categorySelect">
                                @foreach ($categories as $cat)
                                    <option value="{{ $cat->id }}"
                                        {{ in_array($cat->id, is_array($project->category_id) ? $project->category_id : json_decode($project->category_id, true) ?? []) ? 'selected' : '' }}>
                                        {{ $cat->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="card border-0 shadow-sm mb-3">
                        <div class="card-body">
                            <h6 class="fw-semibold mb-2">Gambar Saat Ini</h6>

                            @if ($project->image)
                                <img src="{{ asset('storage/' . $project->image) }}" class="img-fluid rounded mb-2">
                            @else
...
<div class="modal fade" id="cropModal" tabindex="-1">
    ...
    <script>
        tinymce.init({
            ...
            plugins: 'link image lists media table code fullscreen autoresize',
            toolbar: `
               code undo redo | bold italic underline r | blocks font_size_formats | gallery |
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
                    ...
                    .then(data => {
                        ...
                        if (data.success && data.images) {
                            data.images.forEach(img => {
                                ...
                                const fullUrl = `${window.location.origin}/storage/${img}`;
                                col.innerHTML = `
                                    <img src="${fullUrl}" 
                                        class="img-fluid img-thumbnail gallery-image"
                                        data-src="${fullUrl}"
                                        style="cursor:pointer; height:150px; object-fit:cover; width:100%;">
                                    <div class="form-check position-absolute top-0 end-0 m-1">
                                        <input class="form-check-input select-checkbox" type="checkbox" data-src="${fullUrl}">
                                    </div>`;
            ...
            insertBtn.addEventListener('click', function() {
                ...
                selected.forEach(cb => {
                   const src = cb.dataset.src.startsWith('http') 
                                ? cb.dataset.src 
                                : `${window.location.origin}${cb.dataset.src}`;
